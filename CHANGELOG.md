...
### Refactored
- Centralized `.env` file parsing logic into a unified `src/utils/env.js` utility (`parseEnvLine` and `parseEnvContent`).
- Refactored `sort`, `split`, `generate-constants`, and `create` commands to use the centralized parser, improving parsing reliability for values containing `=` and quoted strings.

## [1.2.17] - 2026-08-21

### Added
- Added `-f`/`--force` flag to `generate-constants` command to bypass interactive prompt and allow non-interactive file overwriting (e.g. in CI/CD).

## [1.2.16] - 2026-08-07

### Fixed
- Added `try/catch` error handling for JSON file reading and parsing in `create-from-json` command to prevent process crashes on invalid JSON syntax.
- Fixed nested object and array serialization in `create-from-json` by stringifying complex values instead of producing `[object Object]`.

## [1.2.15] - 2026-08-06

### Refactored
- Implemented robust CLI argument parsing across all commands using `node:util`'s `parseArgs` to correctly handle flags regardless of positional argument placement.

## [1.2.14] - 2026-07-29

### Fixed
- Fixed comment line parsing in `sort` and `split` commands to properly ignore commented lines with leading whitespace (`line.trim().startsWith('#')`).
- Fixed line filtering in `split` command to ensure only valid key-value definition lines containing `=` are processed.

## [1.2.13] - 2026-07-22

### Fixed
- Fixed invalid JavaScript syntax generation in `generate-constants` for keys containing hyphens, dots, or other non-standard identifier characters by safely quoting keys and using bracket property access (`process.env[...]`).

## [1.2.12] - 2026-07-17

### Changed
- Extracted the `generate-constants` command logic from `index.js` into a separate `generate-constants.js` file inside the `commands` directory.

## [1.2.11] - 2026-07-08

### Changed
- Extracted the `delete` command logic from `index.js` into a separate `delete.js` file inside the `commands` directory.

## [1.2.10] - 2026-07-01

### Changed
- Extracted the `sort` command logic from `index.js` into a separate `sort.js` file inside the `commands` directory.
